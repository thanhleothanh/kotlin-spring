...
object TaskTestDataFactory {
    ...
    val taskEntityWithNoID = TaskEntity(title = "Task with no ID", description = "First task", status = TaskStatus.OPEN)
    val taskEntityStatusOpen = TaskEntity(id = 1L, title = "Task One", description = "First task", status = TaskStatus.OPEN)
    val taskEntityStatusDone = TaskEntity(id = 2L, title = "Entity Two", description = null, status = TaskStatus.DONE, completedAt = Instant.now())
}
